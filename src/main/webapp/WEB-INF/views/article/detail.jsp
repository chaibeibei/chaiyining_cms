...
<body>
		...
		<div class="row offset-1" style="margin-top: 15px;">
			<div class="col-7">
				...
				<div style="margin-top: 10px;margin-bottom: 10px;font-weight: bold;color: #777;">
					...
					<span style="font-size: 24px;color: red;" onclick="collectShow();">收藏</span>
				</div>
				<div>
					访问量：${article.hot }
				</div>
	...
	<div class="modal" tabindex="-1" role="dialog" id="collectModal">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-secondary" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" onclick="collect();">确认收藏</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<script type="text/javascript">
		...
		function collect(){
			...
			$.post("/collect/add",{articleId:articleId},function(res){
				if(res.result){
					alert("收藏成功");
